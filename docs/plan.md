...
- **Current phase:** Phase 1 — Data platform & auth
- **Current position:** **P1.3 (Auth + profile wizard) complete & verified.** Per-module namespaced clients (`apps/web/src/supabase.ts`: `patientClient` storageKey `vd-auth-patient`, `doctorClient` `vd-auth-doctor` — §5.2 session separation). Patient module: Google OAuth sign-in → first-run `ProfileWizard` (§5.6) writing own `profiles` (role 'patient') + `patient_details`, 18+ gate client-side via `isAdult` **and** DB-side via BEFORE INSERT/UPDATE trigger (migration `20260709000003_p1_3_patient_age_gate.sql`). Doctor module: email/password `signInWithPassword`, operator-provisioned, no self-signup (footnote), generic invalid-credentials (no enumeration). Shared `useSession`/`useProfile` hooks + `SignOutBar` live in `apps/web/src/auth/` (outside both module dirs, DEC-19). Per-module gzip budgets under DEC-19 cap (patient 74.57 kB, doctor 0.82 kB ≤ 150 kB); import-boundary lint (`no-restricted-imports` patient↛doctor) clean. All strings via `t()` (DEC-4). AC verified: 10/10 headless integration (doctor pw sign-in + wrong-pw reject; patient own-row upsert + RLS read-back; DB age-gate rejects under-18; RLS blocks cross-owner profile_id write) + both anon sign-in screens captured live (Appendix C). Google OAuth consent = manual-verify.
- **Next action:** Begin **P1.4 — Seed + query layer + cross-tenant RLS tests**: seed 2 hospitals (pilot `citycare` + synthetic 2nd tenant), 2 doctors each, sample patients; `packages/api` base query/mutation hooks (keys centralized §13.2); four cross-tenant RLS denial tests in CI. AC: all four denial tests pass; doctor sees only own-hospital (empty) queue.
...
### P1.1 — Schema + RLS  ‖ (migration owner) — ✅ done 2026-07-09
- [x] Migrations for all tables (§8.2): `hospitals, profiles, memberships, patient_details, consults, consult_messages, review_messages, ai_drafts, prescriptions, reviews, mira_feedback, consult_media, consult_events`. (13 tables, verified)
- [x] uuid PKs, `timestamptz` defaults; `consults.status` as Postgres **enum** incl. `abandoned`/`expired`. (10 labels: active,pending_review,approved,rejected,escalated,communicated,closed,superseded,abandoned,expired)
- [x] RLS **enabled default-deny** on every table in the same migration that creates it; additive grant policies per the §8.3 matrix. (13/13 tables RLS-on, 30 policies)
- [x] MVP index set (§8.2) + partial unique index `consults(patient_id,hospital_id) WHERE status IN ('active','pending_review')` (DEC-14). (`consults_one_open_per_patient_hospital_idx`)
- **AC:** ✅ migrations apply forward-only (`supabase db reset` clean); `supabase gen types` committed to `packages/api/src/database.types.ts` and re-export wired in `index.ts`; CI `db-types` job boots the local stack, regenerates, and `git diff --exit-code`s (verified identical → would pass).

### P1.2 — State machine trigger + trace  ‖ ✅ done 2026-07-09
- [x] Status-transition **trigger** enforcing §8.4; rejects any transition not in the machine; writes `status_change` rows to `consult_events`.
- [x] `consult_events` append-only (no UPDATE/DELETE grants to anyone).
- [x] `consult_trace` **view**: time-ordered `UNION ALL` normalized to `(consult_id,hospital_id,at,actor,actor_id,kind,summary,payload)` (§8.5).
- [x] `export_consult_trace(consult_id)` RPC (JSON).
- **AC:** an illegal transition is rejected at the DB; every status change leaves a trace row. — **verified** on fresh `supabase db reset`: legal `active→pending_review→approved` accepted (each wrote a `status_change` trace event); illegal `approved→active` rejected at DB (`illegal consult transition: approved -> active`); no UPDATE/DELETE grants on `consult_events`; `consult_trace` view + `export_consult_trace` RPC each returned the expected trace rows.

### P1.3 — Auth + profile wizard  ‖  ✅ **DONE** (2026-07-09)
- [x] Supabase Auth: Google OAuth (patient module), email/password (doctor module, operator-provisioned, no self-signup).
- [x] Per-module namespaced Supabase client (localStorage key prefix + JWT `role` claim) — app-level session separation on shared origin (§5.2).
- [x] First-run profile wizard → `patient_details` (name, DOB, sex, blood group, allergies, conditions, medications); **18+ DOB enforcement** (§5.6).
- **AC:** patient signs in, completes profile, sees empty records; under-18 DOB rejected. → **met** (profile-write + under-18 trigger + cross-owner RLS all proven headlessly, 10/10 integration; anon sign-in screens captured — Appendix C. Google OAuth consent = manual-verify.)
...
| 2026-07-08 | P0.6 | PWA shell foundation (DEC-17): `vite-plugin-pwa` wired, manifest + service worker (`sw.ts`), offline app-shell precache, installability. build/typecheck/test green. Commit `b1007fe` | Begin P0.7 — CI |
| 2026-07-08 | P0.7 | CI pipeline `.github/workflows/ci.yml` (`verify`: setup-bun@v2 → typecheck→lint→test→build→size; `deploy`: CF Pages, main-only, gated on `CLOUDFLARE_API_TOKEN`). DEC-19 import boundary via `no-restricted-imports`; per-module ≤150 KB gzip via `size-limit`. Lint now runs (`eslint .`). **Phase 0 done**, tag `v0-foundation`, PR #1 merged CI-green. Commits `13df4f9`, `b20eedf` | Begin Phase 1 — P1.1 |
| 2026-07-09 | P1.1 | **Schema + RLS** — one forward-only migration `20260709000001_p1_1_schema_rls.sql` (488 ln): 13 §8.2 tables (uuid PK, `timestamptz`), `consult_status` enum (10 labels incl. `abandoned`/`expired`), `is_member`/`current_role_of` SECURITY DEFINER helpers, `consult_events` append-only trigger, DEC-14 partial unique index, `hospital_public` view, **RLS default-deny on all 13 tables + 30 §8.3 policies**. §8.2 contracts-are-law: `packages/api/src/database.types.ts` (812 ln) committed + re-exported from `index.ts`. New CI `db-types` job regenerates types & fails on drift. Verified on fresh `db reset`: 13/13 RLS-on, 30 policies, DEC-14 index; regenerated types **identical** to committed. typecheck 13/13 | Begin P1.2 — state machine trigger + trace |
| 2026-07-09 | P1.2 | **State machine trigger + Consult Trace** — forward-only migration `20260709000002_p1_2_state_machine_trace.sql` (248 ln): `consult_transition_allowed(from,to)` IMMUTABLE fn (verbatim mirror of `CONSULT_TRANSITIONS`, §13 port-don't-redesign), `consults_enforce_status_machine()` BEFORE-UPDATE-OF-status trigger (SECURITY DEFINER; rejects illegal transitions `errcode=check_violation`, writes `status_change` to `consult_events` w/ actor from `current_role_of()`), explicit `revoke update,delete on consult_events`, `consult_trace` view (`security_invoker`, tenant-isolated `UNION ALL` over 7 event-bearing tables → `(consult_id,hospital_id,at,actor,actor_id,kind,summary,payload)`), `export_consult_trace(uuid)` SECURITY-INVOKER JSON RPC. **5/5 ACs verified** on fresh `db reset`: legal `active→pending_review→approved` accepted + each wrote a trace event; illegal `approved→active` rejected at DB; no UPDATE/DELETE grants on `consult_events`; view + RPC each returned 2 trace rows. §8.2 types regenerated (+21 ln). typecheck 13/13, lint clean, build 7/7 | Begin P1.3 — auth + profile wizard |
| 2026-07-09 | P1.3 | **Auth + profile wizard** — per-module namespaced clients (`apps/web/src/supabase.ts`: `patientClient`/`doctorClient`, distinct `storageKey` `vd-auth-patient`/`vd-auth-doctor`, §5.2). Patient: Google OAuth (`SignIn.tsx`) → first-run `ProfileWizard.tsx` upserting own `profiles` (role 'patient') + `patient_details`; **18+ gate client-side (`isAdult`) + DB backstop** trigger, migration `20260709000003_p1_3_patient_age_gate.sql` (BEFORE INSERT/UPDATE on `patient_details`). Doctor: `signInWithPassword`, operator-provisioned, **no self-signup** (footnote `auth.doctor.noSelfSignup`), generic invalid-credentials (no enumeration). Shared `useSession`/`useProfile`/`SignOutBar` in `apps/web/src/auth/` (outside module dirs, DEC-19); all copy via `t()` (DEC-4). **10/10 headless integration** on live local stack: doctor pw sign-in ok + wrong-pw rejected; patient upserts own `profiles`+`patient_details` & reads back (RLS owner); under-18 DOB rejected by DB trigger; cross-owner `profile_id` write RLS-blocked. Both anon sign-in screens captured from live app (`:5177`): patient "Continue with Google"; doctor "Clinician sign-in" + provisioning footnote. Budgets patient 74.57 kB / doctor 0.82 kB gzip (≤150 kB DEC-19); import-boundary lint clean; typecheck/build green. Google OAuth consent = manual-verify | Begin P1.4 — seed + query layer + cross-tenant RLS tests |
