...
- **Current position:** **P1.2 (State machine trigger + Consult Trace) complete & verified.** Second forward-only migration `supabase/migrations/20260709000002_p1_2_state_machine_trace.sql` (248 lines) adds: `consult_transition_allowed(from,to)` IMMUTABLE fn (verbatim mirror of `CONSULT_TRANSITIONS` in `packages/core/src/state.ts`, §13 port-don't-redesign); `consults_enforce_status_machine()` BEFORE-UPDATE-OF-status trigger (SECURITY DEFINER) that rejects illegal transitions with `errcode=check_violation` and writes a `status_change` row to `consult_events` (actor derived from `current_role_of()`); explicit `revoke update, delete on consult_events` (belt-and-suspenders over P1.1's no-mutate trigger); `consult_trace` view (`security_invoker=true`, tenant-isolated) as a time-ordered `UNION ALL` over consult_events/messages/review_messages/ai_drafts/reviews/prescriptions/consult_media normalized to `(consult_id,hospital_id,at,actor,actor_id,kind,summary,payload)`; `export_consult_trace(uuid)` SECURITY-INVOKER RPC returning the JSON array. All 5 ACs verified on fresh `supabase db reset` (see Appendix C). §8.2 types regenerated (`database.types.ts` +21 lines: the view + RPC), typecheck 13/13, lint clean, build 7/7.
- **Next action:** Begin **P1.3 — Auth + profile wizard** (§5.2/§5.6): Supabase Auth (Google OAuth patient module; email/password doctor module, operator-provisioned, no self-signup); per-module namespaced Supabase client (localStorage prefix + JWT `role` claim); first-run profile wizard → `patient_details` with 18+ DOB enforcement.
