...
- **Current position:** **P1.1 (Schema + RLS) complete & verified.** One forward-only migration `supabase/migrations/20260709000001_p1_1_schema_rls.sql` (488 lines) creates all 13 §8.2 tables (uuid PKs, `timestamptz` defaults), the `consult_status` enum (10 labels incl. `abandoned`/`expired`), helper fns `is_member(uuid)`/`current_role_of()` (SECURITY DEFINER, defined after tables so bodies validate), `consult_events` append-only trigger, DEC-14 partial unique index `consults_one_open_per_patient_hospital_idx`, `hospital_public` view, and enables **RLS default-deny on all 13 tables with 30 additive §8.3 policies**. Empirically verified on a fresh `supabase db reset`: 13/13 tables RLS-on, 30 policies, DEC-14 index present. §8.2 contracts-are-law: `packages/api/src/database.types.ts` (812 lines) committed + re-exported from `index.ts` (`Database`/`Json`/`Tables`/`TablesInsert`/`TablesUpdate`/`Enums`/`CompositeTypes`/`Constants`), typecheck 13/13. New CI `db-types` job boots the local stack (`supabase start -x` heavy services), regenerates, and `git diff --exit-code`s — regenerated types confirmed **identical** to committed (staleness gate would pass). Isolated local stack on 544xx ports (api 54421, db 54422, studio 54423), coexists with the other project's 543xx stack
- **Next action:** Begin **P1.2 — State machine trigger + trace** (§8.4/§8.5): status-transition trigger enforcing the §8.4 machine (rejects illegal transitions at the DB, writes `status_change` rows to `consult_events`), `consult_trace` view (time-ordered UNION ALL), and `export_consult_trace(consult_id)` RPC.
